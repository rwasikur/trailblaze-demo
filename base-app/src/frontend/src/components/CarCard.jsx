...
const CarCard = ({ car, featured = false, isComparing = false, onCompareToggle = () => { } }) => {
    ...
    const discount = car && car.discount_percentage !== undefined ? parseInt(car.discount_percentage) : 0;
    const hasDiscount = car && discount > 0 && discount < 100;
    const discountedPrice = hasDiscount ? Math.round((car.price || 0) - ((car.price || 0) * discount / 100)) : car?.price;

    const priceLabel = discountedPrice ? `$${discountedPrice.toLocaleString()}` : 'Price on request';
    const originalPriceLabel = car.price ? `$${car.price.toLocaleString()}` : '';
            ...
            <Link to={`/car/${car._id}`} className="group block h-full rounded-[1.75rem] focus:outline-none focus-visible:ring-2 focus-visible:ring-accent relative transition-all duration-500">
                ...
                <article id={`car-card-${car._id}`} className={`car-card relative flex h-full flex-col overflow-hidden rounded-[1.75rem] border border-slate-200/80 bg-white shadow-[0_8px_30px_rgb(0,0,0,0.04)] transition-all duration-500 hover:-translate-y-2 hover:shadow-[0_20px_50px_rgba(37,99,235,0.1)] ${featured ? 'lg:flex-row' : ''}`}>
                    <div className={`relative overflow-hidden bg-slate-100 ${featured ? 'lg:w-[52%] aspect-[4/3] lg:aspect-auto' : 'aspect-[16/10]'}`}>
                        ...
                        <div className="absolute top-4 left-4 animate-in fade-in zoom-in duration-500 delay-100 flex flex-col gap-2">
                        ...
                        {hasDiscount && (
                            <div className="absolute top-16 right-4 animate-in fade-in zoom-in duration-500 delay-200">
                                <span id={`car-card-${car._id}-discount-badge`} className="inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-[10px] font-black uppercase tracking-[0.2em] shadow-xl backdrop-blur-md border border-white/20 bg-red-600/90 text-white">
                                    {discount}% OFF
                                </span>
                            </div>
                        )}
                        ...
                        <div className="mt-6 flex flex-col gap-6">
                            <div className={`grid ${car.condition === 'New' ? 'grid-cols-2' : 'grid-cols-3'} gap-2 pb-6 border-b border-slate-100`}>
                                <div className="text-center">
                                    ...
                                    <div className="text-xs font-bold text-slate-900">{car.transmission?.slice(0, 4) || '—'}</div>
                                </div>
                                <div className={`text-center ${car.condition !== 'New' ? 'border-x border-slate-100' : ''}`}>
                                    ...
                                    <div className="text-xs font-bold text-slate-900">{car.seating_capacity || '—'}</div>
                                </div>
                                {car.condition !== 'New' && (
                                    <div className="text-center">
                                        <div className="text-[9px] font-bold uppercase tracking-[0.15em] text-slate-400 mb-1">Miles</div>
                                        <div className="text-xs font-bold text-slate-900">{car.mileage?.split(' ')[0] || '—'}</div>
                                    </div>
                                )}
                            </div>
                            ...
                            <div className="flex items-center justify-between gap-4">
                                <div>
                                    ...
                                    {hasDiscount && (
                                        <div id={`car-card-${car._id}-original-price`} className="text-xs font-bold text-slate-400 line-through">
                                            {originalPriceLabel}
                                        </div>
                                    )}
                                </div>
                                <div className="flex items-center gap-2">
                                    ...
                                    <button
                                        ...
                                        className={`h-12 w-12 rounded-2xl flex items-center justify-center transition-all duration-300 compare-toggle-btn ${isComparing ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/30' : 'bg-slate-100 text-slate-400 hover:bg-slate-200 hover:text-slate-900'}`}
