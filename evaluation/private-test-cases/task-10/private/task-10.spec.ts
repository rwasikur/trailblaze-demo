...
const ADMIN_DASHBOARD_PATH = path.join(REPO_ROOT, 'base-app', 'src', 'frontend', 'src', 'pages', 'AdminDashboard.jsx');
...
async function openDashboardWithToken(page: Page, baseURL: string) {
...
test('AC 17: dashboard currency formatter supports displaying 12345 as $12,345', async () => {
    const source = readSource(ADMIN_DASHBOARD_PATH);
...
test('AC 18: dashboard displays one seeded fleet status legend row per returned status', async ({ page, baseURL }) => {
    const analytics = await getAnalytics(baseURL || '');
    await openDashboardWithToken(page, baseURL || '');
...
// NEGATIVE TESTS (AC 19 - 29)

test('AC 19: backend returns zero-valued fleet metrics when there are zero car records', async () => {
    const source = readSource(ADMIN_CONTROLLER_PATH);
...
test('AC 20: averageFleetAge returns 0 when there are no finite positive model years', async () => {
    const source = readSource(ADMIN_CONTROLLER_PATH);
    expect(source).toContain('validYears.length === 0');
    expect(source).toContain('? 0');
});

test('AC 21: oldestModelYear returns null when there are no finite positive model years', async () => {
    const source = readSource(ADMIN_CONTROLLER_PATH);
    expect(source).toContain('const oldestModelYear = validYears.length ? Math.min(...validYears) : null');
});

test('AC 22: newestModelYear returns null when there are no finite positive model years', async () => {
    const source = readSource(ADMIN_CONTROLLER_PATH);
    expect(source).toContain('const newestModelYear = validYears.length ? Math.max(...validYears) : null');
});

test('AC 23: missing, empty, or unparsable insurance does not increment expiredInsuranceCount', async () => {
    const source = readSource(ADMIN_CONTROLLER_PATH);
...
test('AC 24: missing, empty, or unparsable insurance does not increment insuranceExpiringSoonCount', async () => {
    const source = readSource(ADMIN_CONTROLLER_PATH);
    expect(source).toContain('if (!value) return null');
    expect(source).toContain('Number.isNaN(parsed.getTime()) ? null : parsed');
...
test('AC 25: dashboard has an empty state for empty statusDistribution without a chart', async () => {
    const source = readSource(ADMIN_DASHBOARD_PATH);
...
test('AC 26: dashboard has an empty state for empty fuelTypeDistribution', async () => {
    const source = readSource(ADMIN_DASHBOARD_PATH);
    expect(source).toContain('id="fuel-type-empty"');
    expect(source).toContain('No fuel type data available.');
});

test('AC 27: dashboard displays analytics request failure message', async ({ page, baseURL }) => {
    const token = await getAdminToken(baseURL || '');
    await page.addInitScript(adminToken => {
        window.localStorage.setItem('adminToken', adminToken);
    }, token);
    await page.route('**/api/cars/admin/all', route => route.continue());
...
test('AC 28: Retry sends one additional analytics summary request', async ({ page, baseURL }) => {
    let requestCount = 0;
...
test('AC 29: totalSalesRevenue returns 0 when SaleHistory.sum("price") returns null', async () => {
    const source = readSource(ADMIN_CONTROLLER_PATH);
