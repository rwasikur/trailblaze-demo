...
// Private Configuration - using @pri.com as per private base tests
const USERS = {
...
test('Public users can toggle vehicles for comparison using a dedicated \'Compare\' button on the /browse page, triggering a success toast and a glassmorphic floating bar showing accurate selection counts. (Private)', async ({ page, baseURL }) => {
    ...
    // Expect toast and internal state change
...
test('Selection status persists when navigating between the catalog, individual vehicle details page, and back, with the button state reflecting \'In Comparison\'. (Private)', async ({ page, baseURL }) => {
...
test('Users can add/remove vehicles from comparison directly from the /car/:id overview section using a matching \'Compare\' toggle with toast confirmation on addition. (Private)', async ({ page, baseURL }) => {
    const cars = await getCars(baseURL || '');
    const firstCar = cars[0];
...
test('Comparing 2 or more vehicles generates a technical side-by-side spec table on the /compare page. (Private)', async ({ page, baseURL }) => {
    await page.goto(`${baseURL}/browse`);
    const carCards = page.locator('article[id^="car-card-"]');
...
test('Each column in the comparison matrix features the vehicle\'s high-resolution image and a \'View Masterpiece\' button that successfully lands the user on the specific /car/:id profile. (Private)', async ({ page, baseURL }) => {
...
test('Authenticated administrators can utilize the comparison system while managing the fleet to evaluate market positioning. (Private)', async ({ page, baseURL }) => {
...
test('Users can click a \'Clear Comparison\' button within the floating comparison bar to instantly remove all currently selected vehicles from the comparison list and hide the bar. (Private)', async ({ page, baseURL }) => {
    await page.goto(`${baseURL}/browse`);
    const carCards = page.locator('article[id^="car-card-"]');

    // Select two private-seeded cars
    await carCards.nth(0).locator('.compare-toggle-btn').first().click();
    await carCards.nth(1).locator('.compare-toggle-btn').first().click();
...
test('Attempting to select more than 4 vehicles triggers a warning toast: \'Maximum 4 vehicles can be compared at once\' and blocks the addition. (Private)', async ({ page, baseURL }) => {
    await page.goto(`${baseURL}/browse`);
    const carCards = page.locator('article[id^="car-card-"]');
...
test('Attempting to launch a comparison with fewer than 2 vehicles triggers an info toast: \'Select at least 2 vehicles to compare\' and prevents navigation. (Private)', async ({ page, baseURL }) => {
    await page.goto(`${baseURL}/browse`);
...
test('Direct navigation to /compare with no vehicles selected displays a \'Comparison Empty\' fallback with a Browse Fleet option. (Private)', async ({ page, baseURL }) => {
