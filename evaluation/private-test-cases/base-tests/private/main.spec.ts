...
const SEED_ADMIN = {
    name: 'Private Seed Admin',
    email: 'private-seed-admin@trailblazer-private.test',
    password: 'PrivateSeedPass456!',
};

// Dedicated account for registration UI test — distinct from SEED_ADMIN (Principle 13)
const REG_TEST_EMAIL = 'priv-reg-ui-test@trailblazer-private.test';

// Dedicated account for registration API test — distinct from both above (Principle 13)
const API_TEST_EMAIL = 'priv-reg-api-test@trailblazer-private.test';

const INVALID_CREDENTIALS = {
    email: 'nobody@private-doesnotexist.test',
...
test.describe('Base Application Core Tests (Private)', () => {
    ...
    test('AA-06: Registration with valid credentials returns 201 and redirects to dashboard', async ({ page, baseURL }) => {
        ...
        // Act
        await page.locator('#admin-name-input').fill('Private Registration Test Admin');
        await page.locator('#admin-email-input').fill(REG_TEST_EMAIL);
        await page.locator('#admin-password-input').fill('PrivRegTestPass123!');
    ...
    test('AA-07: Registration API returns 201 and a token for a valid payload', async ({ baseURL }) => {
        ...
        const res = await ctx.post(`${url}/api/admin/signup`, {
            data: { full_name: 'Private API Test Admin', email: API_TEST_EMAIL, password: 'PrivApiTestPass123!' }
        });
