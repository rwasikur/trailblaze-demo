...
test('AC 10: Fleet Expansion: Brand New Vehicle', async ({ page, baseURL }) => {
    ...
    // For 'New' cars, we already default insurance to 1 year from now, 
    // but the test should fill it to be safe (or just click Next if it's already filled)
    // Actually, I'll fill it to ensure the test is robust.
...
test('AC 12: Existing Fleet Data Mutation (Edit Car)', async ({ page, baseURL }) => {
    ...
    await page.locator('label:has-text("Price ($)") + input').fill('60000');
    await page.locator('label:has-text("Seating Capacity") + input').fill('10');
...
test('AC 22: Administrative Registration Workflow', async ({ page, baseURL }) => {
    ...
    await page.locator('#admin-signup-email').fill(`admin_1${Date.now()}@test.com`);
